function lineHandles_h = hline(y, lineType, label, textPosition, axesHandle,linewidt)
...
% Acknowledgement:  It was based on hline() written by Brandon Kuczenski.
    ...
    if( ~exist('linewidt', 'var') )    linewidt = [];      end
    ...
    if isempty(lineType) && isempty(linewidt)
        lineHandles_h = plot(axesHandle, Xlimits, [y';y']);
    elseif ~isempty(lineType) && isempty(linewidt)
        lineHandles_h = plot(axesHandle, Xlimits, [y';y'], lineType);
    elseif ~isempty(lineType) && isempty(linewidt)
        lineHandles_h = plot(axesHandle, Xlimits, [y';y'],'linewidt',linewidt);
    elseif ~isempty(lineType) && ~isempty(linewidt)
        lineHandles_h = plot(axesHandle, Xlimits, [y';y'] ,lineType,'linewidt',linewidt);
    end
    ...
    if( ~isempty(label) )
        ...
        for k=1:length(y)
            ...
            set( textHandles(k), 'color', get(lineHandles_h(k), 'color') );
        end                    
    end
    ...
    set(lineHandles_h,'tag','hline','handlevisibility','off') 
